...
const getProjectById = async (req, res) => {
  try {
    ...
    const userIp = req.headers["x-forwarded-for"]?.split(",")[0] || req.socket.remoteAddress; // User IP
    ...
    const hasViewed = project.viewers.some(viewer => viewer.userId?.toString() === userId || viewer.ip === userIp);
